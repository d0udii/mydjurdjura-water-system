...
async function seedDatabase() {
  ...
  try {
    ...
    // 3. Get region IDs for foreign keys
    ...
    // 4. Add transport tariffs
    console.log('🚚 Adding transport tariffs...')
    const { error: tariffsError } = await supabase
      .from('transport_tariffs')
      .upsert([
        { city: 'Tizi Ouzou', price: 150.00, driver_type: 'local', region_id: regionMap['Tizi Ouzou'] },
        { city: 'Bejaia', price: 200.00, driver_type: 'local', region_id: regionMap['Bejaia'] },
        { city: 'Boumerdes', price: 120.00, driver_type: 'local', region_id: regionMap['Boumerdes'] },
        { city: 'Azazga', price: 180.00, driver_type: 'factory', region_id: regionMap['Djurdjura'] },
        { city: 'Mekla', price: 160.00, driver_type: 'factory', region_id: regionMap['Djurdjura'] }
      ])
    ...
    const { error: clientsError } = await supabase
      ...
      .upsert([
        { 
          ...
          region_id: regionMap['Tizi Ouzou'], 
    ...
    const { error: ordersError } = await supabase
      ...
      .upsert([
        { 
          client_id: clientMap['Restaurant Le Palmier'], 
          region_id: regionMap['Tizi Ouzou'], 
          status: 'pending', 
          total_price: 500.00, 
        ...
        { 
          client_id: clientMap['Hotel Les Pins'], 
          region_id: regionMap['Bejaia'], 
          status: 'in_progress', 
          total_price: 125.00, 
        ...
        { 
          client_id: clientMap['Café Central'], 
          region_id: regionMap['Boumerdes'], 
          status: 'delivered', 
          total_price: 300.00, 
        ...
        { 
          client_id: clientMap['Boulangerie Moderne'], 
          region_id: regionMap['Tizi Ouzou'], 
          status: 'delivered', 
          total_price: 225.00, 
          delivery_date: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], 
        ...
        { 
          client_id: clientMap['Supermarket Express'], 
          region_id: regionMap['Bejaia'], 
          status: 'pending', 
          total_price: 240.00, 
    ...
    // 9. Get order IDs for BL numbers
    const { data: orderData } = await supabase.from('orders').select('id').limit(5)
    ...
    const { error: blError } = await supabase
      .from('bl_numbers')
      .upsert([
        { bl_number: 'BL2024001', order_id: orderIds[0], status: 'active' },
        { bl_number: 'BL2024002', order_id: orderIds[1], status: 'active' },
        { bl_number: 'BL2024003', order_id: orderIds[2], status: 'active' },
        { bl_number: 'BL2024004', order_id: orderIds[3], status: 'active' },
        { bl_number: 'BL2024005', order_id: orderIds[4], status: 'active' }
      ])
    
    if (blError) console.error('BL numbers error:', blError)
    else console.log('✅ BL numbers added')

    // 11. Get user IDs for notifications
    ...
    const { error: notifError } = await supabase
      ...
      .upsert([
        { 
          user_id: userMap['admin@djurdjura.dz'], 
        ...
        { 
          user_id: userMap['ahmed.benali@djurdjura.dz'], 
        ...
        { 
          user_id: userMap['fatima.zohra@djurdjura.dz'], 
        ...
        { 
          user_id: userMap['omar.khelil@djurdjura.dz'], 
    ...
    const { error: logsError } = await supabase
      ...
      .upsert([
        { 
          ...
          affected_record_id: clientMap['Restaurant Le Palmier']
        },
        { 
          user_id: userMap['ahmed.benali@djurdjura.dz'], 
          ...
          affected_record_id: clientMap['Hotel Les Pins']
        }
      ])
