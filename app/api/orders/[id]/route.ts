...
export async function PATCH(
  ...
  try {
    const { id } = await params
    const body = await request.json()
    const { action, user_role, user_id, ...updateData } = body
    ...
    // Check permissions for operations team actions
    if (['approve', 'reject', 'update_bl_number', 'update_status', 'edit'].includes(action)) {
      if (user_role !== 'operations' && user_role !== 'admin') {
        return NextResponse.json(
    ...
    if (action === 'approve') {
      const { bl_number, approved_by } = body
      
      // BL number must be provided by Operations Team
      if (!bl_number) {
        return NextResponse.json(
          { error: 'BL number is required for order approval' },
          { status: 400 }
        )
      }
      ...
      const updatedOrder = {
        ...
        status: 'processing',
        bl_number: bl_number,
        approved_by: approved_by || user_id || 'USR-004',
        approved_at: new Date().toISOString(),
        updated_at: new Date().toISOString()
      }
      ...
      // Create notification for supervisor
      const supervisorNotification = createOrderApprovalNotification(order, order.created_by, bl_number)

      return NextResponse.json({
        order: updatedOrder,
        notification: supervisorNotification,
        message: `Order approved successfully with BL number ${bl_number}`
      })
    }
    ...
    if (action === 'update_status') {
      ...
      const updatedOrder = {
        ...
        status: status,
        updated_at: new Date().toISOString(),
        status_updated_by: user_id || 'USR-004',
        status_updated_at: new Date().toISOString()
      }
    ...
    if (action === 'edit') {
      ...
      // Create notification for supervisor
      const editedFields = Object.keys(body).filter(key => 
        key !== 'action' && key !== 'user_role' && key !== 'user_id' && body[key]
      )
      const supervisorNotification = createOrderEditNotification(order, order.created_by, editedFields)
